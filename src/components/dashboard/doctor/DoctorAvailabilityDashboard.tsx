...
export default function DoctorAvailabilityDashboard() {
  ...
  // المصفوفة المعرفة يدوياً لتفادي أخطاء الـ mapping
  const days = [
    { id: 0, label: t('days.sunday') }, { id: 1, label: t('days.monday') }, { id: 2, label: t('days.tuesday') },
    { id: 3, label: t('days.wednesday') }, { id: 4, label: t('days.thursday') }, { id: 5, label: t('days.friday') }, { id: 6, label: t('days.saturday') }
  ];
  ...
  return (
    <div className={`pt-24 px-4 w-full max-w-5xl mx-auto ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold flex items-center gap-3 text-emerald-700">
          <Stethoscope className="w-8 h-8" /> {t('title')}
        </h1>
        <Button variant="outline" onClick={() => i18n.changeLanguage(i18n.language === 'ar' ? 'en' : 'ar')}>
      ...
      <Card className="mb-8 border-t-4 border-t-emerald-500 shadow-md">
        <CardHeader><CardTitle className="text-emerald-600 flex items-center gap-2"><Calendar className="w-5 h-5"/> {t('add_new')}</CardTitle></CardHeader>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-emerald-50">
              <TableRow>
                <TableHead className="text-emerald-800">{t('day')}</TableHead>
                <TableHead className="text-emerald-800">{t('start')}</TableHead>
                <TableHead className="text-emerald-800">{t('end')}</TableHead>
                <TableHead className="text-center text-emerald-800">{t('actions')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading || isFetching ? (
                ...
                <TableRow key={av.id} className="hover:bg-emerald-50/50 transition-colors">
                  <TableCell className="font-bold text-gray-700">{days.find(d => d.id === av.day_of_week)?.label}</TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-center">
                    {editingId === av.id ? (
                      ...
                      <div className="flex justify-center gap-2">
                        ...
                        <Button size="sm" variant="ghost" className="text-red-600" onClick={() => { if(window.confirm(t('confirm_delete'))) deleteMutation.mutate(av.id, { onSuccess: () => refetch() })}} disabled={deleteMutation.isPending}><Trash className="w-4 h-4" /></Button>
                      </div>
                    )}
                  </TableCell>
                </TableRow>
